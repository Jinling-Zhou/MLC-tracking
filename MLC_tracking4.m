% This is the fourth version of MLC tracking
...
% V2 calculate under-exposed and over-exposed areas, geometric errors and
% latency (latency calculation is refined using xcorr)

% V3 updated ways to calculate under-exposed and over-exposed areas.
% This version used shoelace formula.
% V3.1 is a batch processing program to calculate the tracking results for varied parameters.

% V4 adds latency time that takes MLC to move in place. 
% The targeted MLC locations are compared to the tumor locations happened at the same time stamp.
% latency calculation method changed due to new way of comparison.
% The original method using xcorr() would incur inaccurate results due to the uneven time stamps for new comparison. 
% At places where there is shorter time steps for the trace points, xcorr() evaluate it the same as those with longer time steps. 
% fitting method is used instead, however, for fitting to be correct, it
% has to start with good starting values, such as tumor amplitude and tumor
% freqency. (To keep the original xcorr() codes, the fitting methods are
% updated in the 4_1 version)

% Jinling Zhou, 7/5/2023.
...
% Amplitude_values = [1, 2.5, 5, 7.5, 10, 15, 20];
% Tumor_period_values = [1500, 2000, 4000, 6000, 8000];
% for Image_time = 50:50:300 % in ms
%     for Amplitude = Amplitude_values % in mm
%         for Tumor_period = Tumor_period_values 

% Amplitude_values = 5:5:35;
% Tumor_period_values = 3000:1000:8000;
% for Image_time = 50:50:300 % in ms
%     for Amplitude = Amplitude_values % in mm
%         for Tumor_period = Tumor_period_values 

% Single parameter calculation.
for Image_time = 250 % in ms
    for Amplitude = 20 % in mm
        for Tumor_period = 5000
            for N_period = 5

t = (0:Image_time:N_period*Tumor_period)';
Shift_t =  Amplitude*sin(2*pi*t/Tumor_period);
...
T_move_MLC = zeros(N_control,1);
New_t =zeros(N_control,1);
i_count = 1;
k = 0;
for T_MLC = 0:50:max(t)
    k = k +1;
    if T_MLC >= t(i_count)
    Leaf_A_target = Target_A(i_count,:);
    Leaf_B_target = Target_B(i_count,:);
    [A_position, B_position] = Move_control(Leaf_A_start,Leaf_B_start,Lower_index,Upper_index,Leaf_A_target,Leaf_B_target);
    i_count = i_count+1;
    else 
    Leaf_A_target = Target_A(i_count-1,:);
    Leaf_B_target = Target_B(i_count-1,:);
    [A_position, B_position] = Move_control(Leaf_A_start,Leaf_B_start,Lower_index,Upper_index,Leaf_A_target,Leaf_B_target);
    end
%     fprintf(fileID,'i_count=%.3f, k=%.3f',i_count, k);
%     if i_count <=101
%     fprintf(fileID,'T_MLC=%.3f, t=%.3f',T_MLC, t(i_count));
%     end
...
% Time at which MLC move to place
    A_traveled = max(abs(A_position-Leaf_A_start));
    B_traveled = max(abs(B_position-Leaf_B_start));
    Distance_traveled = max(A_traveled,B_traveled);
    T_move_MLC(k) = Distance_traveled/leafSpeed;
    New_t(k) = T_move_MLC(k)+T_MLC; 

%   for 3D plotting purposes
    for j = 1: length(center_value)
       t_space(k,j) = New_t(k)/1000;
    end  
...
% intropulated tumor location at the time when MLC move to place
% New_Shift_t = interp1(t, Shift_t, New_t); % range of New_shift_t exceeds
% that of New_t due to delay in movement. 
New_Shift_t = Amplitude*sin(2*pi*New_t/Tumor_period);
...
% New_t'/1000,control_trace(:,1),Tumor_trace(:,1));
% perpendicular, shift in x direction. longtitude, shift in y direction
% time_delay is the term used here for latency. 
if angle == pi/2 && Amplitude > 2.5
    % plot(New_t,Tumor_trace(:,1),'*',New_t,control_trace(:,1),'.');
    [correlation, lag] = xcorr(Tumor_trace(:,1), control_trace(:,1));
    [~, maxIndex] = max(correlation);
%     time_step = max(New_t) / (numel(New_t) - 1);
%     time_delay = lag(maxIndex)* time_step; % should be minus, it means how much the second/control_trace is lagging behind the first/Tumor_trace.
time_delay = New_t(-lag(maxIndex));
elseif angle == pi/2 && Amplitude <= 2.5
    time_delay = NaN;
elseif angle == 0
    % plot(New_t,Tumor_trace(:,2),'*',New_t,control_trace(:,2),'.');
    [correlation, lag] = xcorr(Tumor_trace(:,2), control_trace(:,2));
    [~, maxIndex] = max(correlation);

%     time_step = max(New_t) / (numel(New_t) - 1);
%     time_delay = lag(maxIndex)* time_step; % should be minus, it means how much the second/control_trace is lagging behind the first/Tumor_trace.
%    uneven time steps, read directly from the New_t file.  
time_delay = New_t(-lag(maxIndex));
end

ResultData(casenumber).results = struct('TumorMoveAmplitude', Amplitude,'TumorMovePeriod', Tumor_period,'Nperiod',N_period,'ImageUpdate', Image_time,...
    'TreatedArea',Treated_area,'UnderdosedArea',Under_exp_area,'OverExposedArea',Over_exp_area,'GeometricError',Geo_err,'Latency',time_delay,...
    'Angle', angle,'TraceTime',New_t,'TumorTrace', Tumor_trace,'ControlTrace',control_trace);
