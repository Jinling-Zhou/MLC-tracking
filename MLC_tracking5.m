% This is the fifth version of MLC tracking
...
% This version is based on the batch processing program for V3 to calculate the tracking results for varied parameters.
% This version reverted back to version 3 for which the time that takes MLC to
% its control locations is not counted. 

% However, latency calculations changed to fitting method. 

% Jinling Zhou, 7/10/2023.
...
% Tumor_period_values = 1000* [8, 7, 6, 5, 4,3];
% for Image_time = 50:50:300 % in ms
%     for Amplitude = Amplitude_values % in mm
%         for Tumor_period = Tumor_period_values 
%            for  N_period = 5
...
% latency calculation
fitting_x = New_t'/1000;
if angle == pi/2
fitting_y = control_trace(:,1); 
elseif angle ==0
fitting_y = control_trace(:,2);
end
% Define the objective function
fun = @(params, xdata) params(1)*sin(params(2)*xdata + params(3));

% Set initial parameter values
params0 = [Amplitude, 2*pi*1000/Tumor_period, 0];  % Initial guess for the amplitude and phase shift

% Optimize the model
options = optimoptions(@fmincon,'Display','iter','FunctionTolerance',1e-6);
params = lsqcurvefit(fun, params0, fitting_x, fitting_y,[],[],options);

% Extract the fitted parameters
amplitude = params(1);
angular_freq = params(2);
phaseShift = params(3);
time_delay = -phaseShift*1000;

% % Latency calculation using xcorr(), New_t/1000 (s) is the time stamps
