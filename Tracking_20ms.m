% This is from the Third version of MLC tracking
...
% This is a batch processing program to calculate the tracking results for varied parameters.
% This is to calculate the senarios for Lei Zhang's research.
% 50 ms control points updated to 20 ms. 

% Jinling Zhou, 6/14/2023.
...
% for Image_time = 50 % in ms
%     for Amplitude = 7.5 % in mm
%         for Tumor_period = 3000:1000:8000


t = (0:Image_time:5*Tumor_period)';
...
% MLC control point at every 20 ms, to move MLC to the most updated target location.  
i = 1;
Leaf_A_start = Leaf_A_o;
Leaf_B_start = Leaf_B_o;
k = 0;
N_control = max(t)/20 + 1;
...
% intropulated tumor location every 20ms
New_t = 0:20:max(t);
...
ResultData(casenumber).results = struct('TumorMoveAmplitude', Amplitude,'TumorMovePeriod', Tumor_period,'ImageUpdate', Image_time,...
...
Length_max = Speed_max * 20; % 1.25 mm
