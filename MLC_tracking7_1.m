% Seventh version of MLC tracking

% V1 created a MLC squence to track tumor movement, without prediction filter.
% Conditions: Max MLC leaf speed: 25 mm/s; Control points at every Tcontrol.
...
% This one uses shoelace formula.

% V6 is created based on V3. The time takes MLC to place for each duration
% of Tcontrol was not calculated in this version. However, the real tumor trace was updated.

% V7 is the patch processing of V6

% Jinling Zhou, 2/7/2024.
...
T_image_values = 50:50:300; % in ms
Amplitude_values = 5:5:35; % in mm
Tumor_period_values = 1000*[3, 4, 5, 6, 7, 8]; % in ms

% Parameters:
for T_image = T_image_values % in ms
    for Amplitude = Amplitude_values % in mm
        for Tumor_period = Tumor_period_values % in ms
            for N_period = 5
                for Tcontrol = 50 % in ms
                
t = (0:T_image:N_period*Tumor_period)';
Shift_t =  Amplitude*sin(2*pi*t/Tumor_period);

% moving angle with respect to edge direction
Speed = Amplitude*400/Tumor_period; 
if Speed > 2.5
        angle = acos(2.5/Speed);
...
% MLC control point at every Tcontrol, to move MLC to the most updated target location.  
i = 1;
Leaf_A_start = Leaf_A_o;
Leaf_B_start = Leaf_B_o;
k = 0;
N_control = floor(max(t)/Tcontrol) + 1;
...
for T_MLC = 0:Tcontrol:max(t)
    k = k +1;
    if T_MLC >= t(i)
    Leaf_A_target = Target_A(i,:);
    Leaf_B_target = Target_B(i,:);
    [A_position, B_position] = Move_control(Leaf_A_start,Leaf_B_start,Lower_index,Upper_index,Leaf_A_target,Leaf_B_target,Tcontrol);
    % fprintf(fileID, 'T_MLC= %.3f, t =%.3f',T_MLC,t(i));
    i = i+1;
    else 
    Leaf_A_target = Target_A(i-1,:);
    Leaf_B_target = Target_B(i-1,:);
    [A_position, B_position] = Move_control(Leaf_A_start,Leaf_B_start,Lower_index,Upper_index,Leaf_A_target,Leaf_B_target,Tcontrol);
    fprintf(fileID, 'T_MLC= %.3f, t =%.3f',T_MLC,t(i-1));
    end
...
% intropulated tumor location every Tcontrol
% This is the update for V6 from V3. 
% Because 'real' tumor size/location value aren't calculated beyond
% max(t) - tshift, the comparison will be done up to max(t) - tshift
tshift = t(2) - t(1);
New_t = 0:Tcontrol:max(t);
New_Shift_t = interp1(t - tshift, Shift_t, New_t); % Interpolate new tumor movement values at new time points
...
N_compare = N_control - ceil(tshift/Tcontrol);
...
% Treated_area = zeros(N_compare,1);
% Over_exp_area = zeros(N_compare,1);
% Under_exp_area = zeros(N_compare,1);
% for i = 1: N_compare
% %        i = CNumber;
%     for j = 1: N_leaf
...
% Geometric error
control_compare = control_trace(1:N_compare,:);
Tumor_compare = Tumor_trace(1:N_compare, :);
Compare_t = New_t(:,1:N_compare);
Geo_diff = Tumor_compare - control_compare;
...
%     moveaxis = 2;
% end
% % plot(New_t,Tumor_trace(:,moveaxis),'*',New_t,control_trace(:,moveaxis),'.');
% [correlation, lag] = xcorr(Tumor_compare(:,moveaxis), control_compare(:,moveaxis));
...
% ResultData(casenumber).results = struct('TumorMoveAmplitude', Amplitude,'TumorMovePeriod', Tumor_period,'Nperiod',N_period,'ImageUpdate', T_image,...
...
%     'Angle', angle,'TraceTime',Compare_t,'TumorTrace', Tumor_compare,'ControlTrace',control_compare);
...
%     y2_A = A_control(m,:);
%     y2_B = B_control(m,:);
%     x3 = control_trace(m,1);
%     y3 = control_trace(m,2);
%     x4 = Tumor_trace(m,1);
%     y4 = Tumor_trace(m,2);
% 
%     plot([x1,x1], [y1_A,y1_B],'og',[x2,x2],[y2_A,y2_B],'*m',x3,y3,'*m',x4,y4,'og');
%     legend({'Tumor','MLC aperture'});
%     axis equal;
% 
...
function [A_position, B_position] = Move_control(Leaf_A_start,Leaf_B_start,I_lower,I_upper,Leaf_A_target, Leaf_B_target,deltaT)
A_position = Leaf_A_start;
B_position = Leaf_B_start;
Speed_max = 0.025; % in mm/ms
Length_max = Speed_max * deltaT; % 1.25 mm
